...
Variable *_A = createVector(100);
Variable *_fzxc0WG0lVAj6AT = generateScalarFromNumber(1);
Variable *_jjX2A9VNFsfJ4qH = generateScalarFromNumber(100);
Variable *_LoGBufBRBHNmNqf = generateScalarFromNumber(1);
for(assign(_i,_fzxc0WG0lVAj6AT); isLower(_i,_jjX2A9VNFsfJ4qH); increase(_i,_LoGBufBRBHNmNqf)){
Variable *_XqfK9Nxwuggw4mF = generateScalarFromNumber(1);
assignToIndex(_A, _i, _XqfK9Nxwuggw4mF);
}
Variable *_gqHSf71iezcBdjb = generateScalarFromNumber(1);
Variable *_EHKvrLK5t5pJqPy = generateScalarFromNumber(0);
assignToIndex(_A, _gqHSf71iezcBdjb, _EHKvrLK5t5pJqPy);
Variable *_ehHGCc0Fy6fV74l = generateScalarFromNumber(2);
Variable *_olx4OhLjzJCaRYs = generateScalarFromNumber(100);
Variable *_gZQKLSjEy4CZwcD = generateScalarFromNumber(1);
Variable *_VRab5y965CSUrp2 = multiplication(_i, _i);
Variable *_649VOR3g7qLY34A = generateScalarFromNumber(100);
for(assign(_i,_ehHGCc0Fy6fV74l); isLower(_i,_olx4OhLjzJCaRYs); increase(_i,_gZQKLSjEy4CZwcD)){
for(assign(_j,_VRab5y965CSUrp2); isLower(_j,_649VOR3g7qLY34A); increase(_j,_i)){
Variable *_AQWygqUNk06ONHx = generateScalarFromNumber(0);
assignToIndex(_A, _j, _AQWygqUNk06ONHx);
}}
Variable *_BBKNLvwJByKkPJn = generateScalarFromNumber(1);
Variable *_kGR7Q0B0MwbPNRB = generateScalarFromNumber(100);
Variable *_7BJI6dqpRz1glsX = generateScalarFromNumber(1);
for(assign(_i,_BBKNLvwJByKkPJn); isLower(_i,_kGR7Q0B0MwbPNRB); increase(_i,_7BJI6dqpRz1glsX)){
printsep();
Variable *_JsDClAyQ67Rr71R = getSingleIndex( _A, _i );
print(_JsDClAyQ67Rr71R);
print(_i);
printsep();
}
return 0;

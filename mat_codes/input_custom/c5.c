...
int main(){
Variable *_A = createMatrix(5, 5);
Variable *_v = createVector(3);
Variable *_i = createScalar();
Variable *_j = createScalar();
Variable *_k = createScalar();
assignToFlatten(_v, 1, 0);assignToFlatten(_v, 5, 1);assignToFlatten(_v, 1, 2);
Variable *_fzxc0WG0lVAj6AT = generateScalarFromNumber(1);
assign(_k, _fzxc0WG0lVAj6AT);
Variable *_LoGBufBRBHNmNqf = generateScalarFromNumber(1);
Variable *_jjX2A9VNFsfJ4qH = getSingleIndex( _v, _LoGBufBRBHNmNqf );
Variable *_gqHSf71iezcBdjb = generateScalarFromNumber(2);
Variable *_XqfK9Nxwuggw4mF = getSingleIndex( _v, _gqHSf71iezcBdjb );
Variable *_ehHGCc0Fy6fV74l = generateScalarFromNumber(3);
Variable *_EHKvrLK5t5pJqPy = getSingleIndex( _v, _ehHGCc0Fy6fV74l );
Variable *_gZQKLSjEy4CZwcD = generateScalarFromNumber(1);
Variable *_olx4OhLjzJCaRYs = getSingleIndex( _v, _gZQKLSjEy4CZwcD );
Variable *_649VOR3g7qLY34A = generateScalarFromNumber(2);
Variable *_VRab5y965CSUrp2 = getSingleIndex( _v, _649VOR3g7qLY34A );
Variable *_BBKNLvwJByKkPJn = generateScalarFromNumber(3);
Variable *_AQWygqUNk06ONHx = getSingleIndex( _v, _BBKNLvwJByKkPJn );
for(assign(_i,_jjX2A9VNFsfJ4qH); isLower(_i,_XqfK9Nxwuggw4mF); increase(_i,_EHKvrLK5t5pJqPy)){
for(assign(_j,_olx4OhLjzJCaRYs); isLower(_j,_VRab5y965CSUrp2); increase(_j,_AQWygqUNk06ONHx)){
assignToDoubleIndex(_A, _i, _j, _k);
Variable *_kGR7Q0B0MwbPNRB = getDoubleIndex( _A, _i, _j );
Variable *_JsDClAyQ67Rr71R = generateScalarFromNumber(1);
Variable *_7BJI6dqpRz1glsX = addition(_JsDClAyQ67Rr71R, _kGR7Q0B0MwbPNRB);
assign(_k, _7BJI6dqpRz1glsX);
}}
print(_A);
printsep();
print(_k);

...
Variable *AITGDPHCSPIjtHb = substraction(n, filterSize);
Variable *KlrwqAOzMiXrpif = generateScalarFromNumber(1);
Variable *sFyfvlADzPBfudk = addition(KlrwqAOzMiXrpif, AITGDPHCSPIjtHb);
Variable *EffECLhbVFUkBye = generateScalarFromNumber(1);
Variable *qfQOJWTwosILEeZ = generateScalarFromNumber(1);
Variable *TxWJLkNgbqQmBxQ = substraction(n, filterSize);
Variable *YSSYoQcJomwUFBd = generateScalarFromNumber(1);
Variable *CQptkhHqrQdwfcA = addition(YSSYoQcJomwUFBd, TxWJLkNgbqQmBxQ);
Variable *fXudZHiftaKCZVH = generateScalarFromNumber(1);
for(assign(i,rCRwDnXeuOQqekl); isLower(i,sFyfvlADzPBfudk); increase(i,EffECLhbVFUkBye)){
for(assign(j,qfQOJWTwosILEeZ); isLower(j,CQptkhHqrQdwfcA); increase(j,fXudZHiftaKCZVH)){
...
Variable *IkFUXwjHTUjRLIu = generateScalarFromNumber(0);
Variable *aifPpzVkmXyspWV = generateScalarFromNumber(0);

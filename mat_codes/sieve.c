...
assignToIndex(primes, lrfkQyuQFjKXyQV, NRTySFrzrmzlYGF);
Variable *vEulQfpDBHlqDqr = generateScalarFromNumber(2);
Variable *rCRwDnXeuOQqekl = generateScalarFromNumber(0);
assignToIndex(primes, vEulQfpDBHlqDqr, rCRwDnXeuOQqekl);
Variable *AITGDPHCSPIjtHb = generateScalarFromNumber(2);
Variable *sFyfvlADzPBfudk = generateScalarFromNumber(100);
Variable *KlrwqAOzMiXrpif = generateScalarFromNumber(1);
Variable *EffECLhbVFUkBye = addition(i, i);
Variable *qfQOJWTwosILEeZ = generateScalarFromNumber(100);
for(assign(i,AITGDPHCSPIjtHb); isLower(i,sFyfvlADzPBfudk); increase(i,KlrwqAOzMiXrpif)){
for(assign(j,EffECLhbVFUkBye); isLower(j,qfQOJWTwosILEeZ); increase(j,i)){
print(i);
Variable *TxWJLkNgbqQmBxQ = getSingleIndex( primes, i );
print(TxWJLkNgbqQmBxQ);
printsep();
Variable *CQptkhHqrQdwfcA = getSingleIndex( primes, i );
Variable *YSSYoQcJomwUFBd = getSingleIndex( primes, j );
Variable *fXudZHiftaKCZVH = getSingleIndex( primes, j );
Variable *PRbsshSjXDFileB = generateScalarFromNumber(1);
Variable *sYBlOetsWCRFHPX = choose( CQptkhHqrQdwfcA, PRbsshSjXDFileB, YSSYoQcJomwUFBd, fXudZHiftaKCZVH );
assignToIndex(primes, j, sYBlOetsWCRFHPX);
}}
Variable *xwBCTOaYaxzFBjb = generateScalarFromNumber(1);
Variable *KRxIRImqPZWMSHl = generateScalarFromNumber(100);
Variable *PJhtAZhbuXhwaDL = generateScalarFromNumber(1);
for(assign(i,xwBCTOaYaxzFBjb); isLower(i,KRxIRImqPZWMSHl); increase(i,PJhtAZhbuXhwaDL)){
Variable *PTOyeziWkMGSovq = getSingleIndex( primes, i );
...
Variable *zGDIxrPDdZpLcrW = choose( PTOyeziWkMGSovq, nqwQeCYjyIBfjyk, MjFQwlTvZKQTPVO, LPHCkCYuFdqMlgl );
print(zGDIxrPDdZpLcrW);
}
return 0;
}
